...
> **⚠️ Giới hạn — TCC chỉ quản lý các phiên tmux, KHÔNG lưu trữ/đóng băng terminal.**
> Chương trình **chỉ quản lý các phiên tmux**, không có khả năng lưu trữ hay
> "đóng băng" các phiên terminal của máy bạn. Các phiên tmux chỉ tồn tại trong
> bộ nhớ của tmux server đang chạy. Vì vậy khi **tắt máy**, hoặc **crash/restart**
> một service liên quan đến TCC, tmux hoặc terminal, thì **toàn bộ các phiên tmux
> hiện tại sẽ bị mất**. TCC không thể khôi phục nội dung/tiến trình của các phiên
> đã mất. Nếu cần dữ liệu sống sót qua khởi động lại, hãy tự lưu ra file/log
> trong chính phiên làm việc.
...
> ([Windows-Terminal-Control-Center](https://github.com/vudinhhung513/Windows-Terminal-Control-Center)). Xem [Roadmap](./docs/ROADMAP.md).
...
- **Thanh nút điều khiển** terminal: cuộn lên/xuống, Enter, ESC, Ctrl+C, Tab, mũi tên, copy/paste
- **Ô nhập liệu mobile** (mặc định trên điện thoại): chạm vào terminal hiện ô textbox ngay trên bàn phím để soạn cả đoạn (gõ được **tiếng Việt** trên iPhone), rồi **chèn** vào terminal một lần (không kèm Enter)
...
| `multiDeviceMode` | Xử lý khi mở một phiên đang dùng ở thiết bị khác (dashboard báo *attached*): `takeover` (**mặc định**) thiết bị mới cướp quyền, thiết bị cũ bị ngắt ra; `lock` chặn thiết bị mới, giữ phiên ở thiết bị đang dùng | `"takeover"` |
| `logging.mode` | Ghi log terminal: `off` (tắt) / `input` (chỉ dòng lệnh, best-effort) / `full` (input + output, đã strip ANSI). **Cảnh báo:** có thể ghi lại mật khẩu gõ ở prompt | `"off"` |
| `logging.retentionDays` | Số ngày giữ log (tính từ lần active cuối của phiên); quá hạn sẽ tự xoá | `7` |
...
- Chọn **truy cập đa thiết bị** (`multiDeviceMode`): *Cướp quyền* (mặc định — thiết bị mới mở phiên sẽ ngắt thiết bị cũ) hoặc *Khoá* (chặn thiết bị mới khi phiên đang dùng ở nơi khác).
- Đặt **thư mục mặc định** cho phiên mới (`defaultPath`) — validate khi lưu: phải là đường dẫn tuyệt đối, tồn tại và là thư mục.
- Cấu hình **ghi log terminal** (`logging`): mức độ (Tắt / Chỉ input / Đầy đủ) và số ngày lưu. **Cảnh báo:** bật log có thể ghi lại mật khẩu gõ ở prompt; log lưu tại `data/logs/`.
