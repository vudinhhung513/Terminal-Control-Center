...
| `mobileKeyboardMode` | Cách xử lý bàn phím ảo trên mobile: `input` (**mặc định**) hiện ô nhập liệu riêng phía trên bàn phím — soạn cả đoạn rồi **chèn** vào terminal (không kèm Enter), gõ được tiếng Việt trên iPhone; `resize` thu nhỏ terminal cho vừa vùng hiển thị còn lại (gõ trực tiếp, IME tiếng Việt có thể không chạy trên iOS) | `"input"` |
...
- Chọn **chế độ bàn phím mobile** (`mobileKeyboardMode`): *Hiện ô nhập liệu* (mặc định — soạn rồi chèn vào terminal, không kèm Enter) hoặc *Thu nhỏ terminal cho vừa* — đổi xong cần mở lại terminal.
